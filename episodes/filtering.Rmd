...
valid_age_dass |>
  ggplot(aes(x = age)) +
  geom_density()  # This will cause an error!
```
...
valid_age_dass |>
  ggplot(aes(x = age)) +
  geom_histogram(50)  # This will cause an error!
```

`geom_histogram` does not expect the number of bins to be the first argument! Therefore, we need to declare this properly:
```{r}
valid_age_dass |>
  ggplot(aes(x = age)) +
  geom_histogram(n_bins = 50)  # This will cause an error!
```
...
::: challenge
## Challenge 1
Maybe filter country, for two distinct values
...
::: challenge
## Challenge 3
What is the argument order in some function?
